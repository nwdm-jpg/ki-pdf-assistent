...
The auth stack is `auth.py` (Argon2 hashing/validation, no Streamlit/DB imports) + `speicher.py` (all persistence: `benutzer`, `email_verifications`, `password_resets`, `sessions`, `security_events` tables) + `benutzer.py` (login/register/forgot-password/reset UI, session lifecycle) + `konto.py` (profile/password/email-change/export/delete UI) + `ratenbegrenzung.py` (persistent rate limiting) + `sicherheitslog.py` (audit logging) + `email_versand.py` (transactional email, dev or Resend). This is still a single-app, local-SQLite MVP — see "Platform & Product Branding" for the intended future evolution into a shared, cross-product **Clevoriq Account** (SSO across future Clevoriq products is explicitly **not implemented**; nothing here should be mistaken for it). **2FA/TOTP is a deliberately separate, not-yet-built follow-up** — do not add it as part of touching this section.

...
AVENLOQ is the central platform/company brand; the account is an AVENLOQ account, not tied to one product. The application in this repo is **AVENLOQ Documents**, the first (currently only) product on that platform. Visual hierarchy is always AVENLOQ dominant, product name smaller/subtler beneath it (`komponenten.marke_kopf`'s `produkt` parameter, default `komponenten.PRODUKT_NAME = "Documents"`, rendered via `.avq-marke-produkt` in `_CSS`) — never the other way round, and never two products shown at once.

The AVENLOQ account (`benutzer.py`/`speicher.py`) is intended to eventually be shared across future AVENLOQ products (e.g. a conceptual "AVENLOQ Invoice" or "AVENLOQ Vault"). Cross-product single sign-on is **not implemented** — sessions are per-app `st.session_state` only (see `benutzer.py`'s module docstring). Do not build SSO, a product switcher, additional products, or placeholders for them unless explicitly requested; `marke_kopf`'s `produkt` parameter exists solely so a future product can reuse the same wordmark component without changing this one, not as an invitation to add one now.

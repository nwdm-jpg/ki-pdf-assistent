...
- **Disclaimers**: legal/analysis disclaimers (`analyse.RISIKEN_HINWEIS`, `pruefung.PRUEFUNG_HINWEIS`) render via `komponenten.hinweis_dezent`, a subdued blue-accent notice — not `st.warning` — to stay professional rather than alarming. Never add "100% secure" / "DSGVO-konform" / "rechtssicher" style claims; auth, hosting, and the final privacy/security architecture don't exist yet.
